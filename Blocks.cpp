...
Blocks::Blocks(int xWidnowSize, int yWindowSize)
{
    this->xWindowSize = xWidnowSize;
...
void Blocks::setSingleRowOfBlocks(int blocksRowShift)
{
    ...
    for(int i=0;i<numberOfRectanglesInSingleRow;i++)
    {
        sf::RectangleShape block;
        block.setSize({xBlockSize, yBlockSize});
        block.setFillColor({200, 110, 100});
        block.setPosition(xPositionOfBlock+i*xBlockSize+leftFreeSpace/2, yPositionOfBlock+blocksRowShift*yBlockSize);
